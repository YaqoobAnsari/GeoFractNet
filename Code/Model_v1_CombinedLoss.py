...
class RobustTLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        diff = inputs - targets
        loss = torch.log(1 + (diff ** 2) / (self.beta ** 2))
        return torch.mean(loss)

class CombinedLoss(nn.Module):
    def __init__(self, alpha=1, gamma=2, beta=1.0):
        super(CombinedLoss, self).__init__()
        self.focal_loss = FocalLoss(alpha=alpha, gamma=gamma)
        self.robust_t_loss = RobustTLoss(beta=beta)

    def forward(self, inputs, targets):
        focal_loss = self.focal_loss(inputs, targets)
        robust_t_loss = self.robust_t_loss(inputs, targets)
        return focal_loss + robust_t_loss
...
if __name__ == "__main__":
    ...
    lr = 0.002
    num_epochs = 125
    criterion = CombinedLoss(alpha=3, gamma=2, beta=3.0)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=5, verbose=True, threshold=0.01, threshold_mode='abs', cooldown=0, min_lr=1e-6)
    ...
    print(f"Training Info: lr = {lr}, Epochs = {num_epochs}, Model = UNet, Optimizer = Adam")
    ...
    model_info = f"UNet1_combloss_lr{lr}_epochs{num_epochs}"
    ...
    for epoch in range(num_epochs):
        ...
        with open(csv_filename, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([epoch+1, epoch_time, epoch_loss, val_loss, accuracy, precision, recall, f1, iou, current_lr, "CombinedLoss", "Adam"])
